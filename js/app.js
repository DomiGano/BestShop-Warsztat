...
selectDropdown.forEach( function (element) {
    element.addEventListener("click", function() {
        ...
        if(element.innerText == "Professional") {
            ...
            calcSummary.querySelectorAll("[data-id='package'] span").forEach(function (element ) {
                if(element.className == "item__calc") {
                    element.innerText = "Professional"
                } if(element.className == "item__price") {
...
ordersInput.addEventListener("input", function () {
    calcSummary.querySelectorAll("[data-id='orders'] span").forEach(function (element ) {
        if(ordersInput.value > 0) {
            ...
            if(element.className == "item__calc") {
                element.innerText = ordersInput.value + " * $1"
            } if(element.className == "item__price") {
                element.innerText = "$" + ordersInput.value * 1 
            }
        } else {
            calcSummary.querySelector("[data-id='orders']").classList.remove("open")
...
checkboxes.forEach(function(element) {
    element.addEventListener("click", function() {
        if(element.id == "accounting") {
            calcSummary.querySelector("[data-id='accounting']").classList.toggle("open")  
        } if(element.id == "terminal") {
            calcSummary.querySelector("[data-id='terminal']").classList.toggle("open")  
        }
    })
})